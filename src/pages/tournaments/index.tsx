...
import { getTournaments, deleteTournamentById } from 'apiSdk/tournaments';
...
function TournamentListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TournamentInterface[]>(
    ...
    () =>
      getTournaments({
        relations: ['organization', 'team.count'],
      }),
  );
  ...
  const handleView = (id: string) => {
    if (hasAccess('tournament', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/tournaments/view/${id}`);
    }
  };
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('tournament', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/tournaments/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('tournament', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/tournaments/edit/${record.id}`} passHref legacyBehavior>
                          <Button
                      ...
                      {hasAccess('tournament', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                        <IconButton
                          onClick={(e) => {
                            e.stopPropagation();
