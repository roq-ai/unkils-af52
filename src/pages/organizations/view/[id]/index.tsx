...
import { deleteTournamentById } from 'apiSdk/tournaments';
...
function OrganizationViewPage() {
  ...
  const tournamentHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('tournament', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/tournaments/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.tournament?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/tournaments/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            <Td>{record.format}</Td>
                            ...
                            <Td>
                              {hasAccess('tournament', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/tournaments/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('tournament', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    tournamentHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
