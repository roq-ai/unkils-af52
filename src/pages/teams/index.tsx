...
function TeamListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TeamInterface[]>(
    ...
    () =>
      getTeams({
        relations: ['tournament', 'player.count'],
      }),
  );
          ...
          <TableContainer>
            <Table variant="simple">
              <Thead>
                <Tr>
                  <Th>name</Th>
                  {hasAccess('tournament', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>tournament</Th>}
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('tournament', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/tournaments/view/${record.tournament?.id}`}>
